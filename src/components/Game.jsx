...
export default function Game() {
  ...
  // Refs for dynamic game parameters
          ...
          .map((pipe) => {
            ...
            if (newPipe.x < BIRD_X && !newPipe.passed) {
              setScore((prev) => {
                ...
                if (newScore % 10 === 0) {
                  gameSpeedRef.current += 0.5; // faster pipes
                  gravityRef.current += 0.001; // bird falls faster
                  pipeIntervalRef.current = Math.max(800, pipeIntervalRef.current - 200); // faster pipe spawn
                  jumpRef.current += 5; // slightly stronger jump
                  clearInterval(pipeInterval); // reset interval with new speed
